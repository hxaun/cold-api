...
func TestInjectSiteTitle(t *testing.T) {
	t.Run("replaces_title_with_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>ColdAPI - AI API Gateway</title></head><body></body></html>`)
		...
		assert.NotContains(t, string(result), "ColdAPI")
	})
	...
	t.Run("returns_unchanged_when_title_has_attributes", func(t *testing.T) {
		...
		html := []byte(`<html><head><title lang="en">ColdAPI</title></head><body></body></html>`)
	...
	t.Run("preserves_rest_of_html", func(t *testing.T) {
		html := []byte(`<html><head><meta charset="UTF-8"><title>ColdAPI</title><script src="app.js"></script></head><body><div id="app"></div></body></html>`)
