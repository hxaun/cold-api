    ...
    <BaseDialog
      ...
      :title="
        selectedAccountsGroup
          ? `${selectedAccountsGroup.name} - 账号信息`
          : '账号信息'
      "
      width="wide"
    ...
    >
      <div class="space-y-4">
          ...
          <div class="max-h-[60vh] overflow-auto">
            <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-dark-600">
              <thead class="sticky top-0 bg-gray-50 dark:bg-dark-700">
                <tr>
                  <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-300">名称</th>
                  <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-300">类型</th>
                  <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-300">状态</th>
                  <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-300">调度</th>
                  <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-300">优先级</th>
                  <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-300">并发</th>
                  <th class="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-300">上次使用</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 bg-white dark:divide-dark-700 dark:bg-dark-800">
                ...
                >
                  ...
                  <td class="px-4 py-3">
                    ...
                    <div
                      v-if="isAccountRateLimited(account)"
                      class="mt-1 text-xs text-amber-600 dark:text-amber-400"
                    >
                      限流至 {{ formatAccountDateTime(account.rate_limit_reset_at) }}
                    </div>
                  </td>
                  <td class="px-4 py-3">
                    <span
                  ...
                  <td class="px-4 py-3 text-gray-500 dark:text-gray-400">
                    {{ formatAccountDateTime(account.last_used_at) }}
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div
          ...
          class="text-xs text-gray-500 dark:text-gray-400"
        >
          当前显示 {{ accountsModalAccounts.length }} 个账号，共 {{ accountsModalTotal }} 个
        </div>
      </div>
    </BaseDialog>
...
const formatAccountDateTime = (value: string | null | undefined): string => {
  if (!value) return "-";
